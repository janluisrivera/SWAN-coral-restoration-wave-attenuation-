%% Figure 6: Acropora palmata spatial wave attenuation
...
offshoreBoundaryHs = [1.0 1.0 3.0];

speciesFolder = 'A.palmata_3.0';
speciesLabel = '\itAcropora palmata';
figureNumber = 6;
...
boundaryEquivalentLimits = [0 0.25];
...
if isfile(bathymetryFile)
    bathymetry = loadGridMatrix(bathymetryFile, nRows, nCols, X);
end

restorationMask = false(nRows, nCols);
if isfile(restorationMaskFile)
    restorationMask = loadMask(restorationMaskFile, nRows, nCols, X);
end

selected = struct();
for s = 1:numel(scenarioFolders)
    [hsYear0, directionYear0, timeYear0] = loadHsDirection(scenario(s).year0);
    ...
    selected(s).hsYear0 = getAtTime(hsYear0, timeYear0, targetTime);
    selected(s).directionYear0 = getAtTime(directionYear0, timeYear0, targetTime);
    selected(s).hsRestored = getAtTime(hsRestored, timeRestored, targetTime);
    ...
    attenuationPercent = 100 .* deltaHs ./ selected(s).hsYear0;
    attenuationPercent(selected(s).hsYear0 < minimumHs) = NaN;
    attenuationPercent(~isfinite(attenuationPercent)) = NaN;

    selected(s).boundaryEquivalent = ...
        (attenuationPercent ./ 100) .* offshoreBoundaryHs(s);
end
...
axisWidth = (1 - leftMargin - rightMargin - 2 * columnGap) / 3;
axisHeight = (1 - bottomMargin - topMargin - 2 * rowGap) / 3;
...
for s = 1:numel(scenarioFolders)
    ...
    elseif useHurricanePercentile
        hsLimits = hurricaneColorLimits( ...
            selected(s).hsYear0, selected(s).hsRestored, hurricanePercentile);
    else
        hsLimits = [0 4];
    end

    for row = 1:3
        xPosition = leftMargin + (s - 1) * (axisWidth + columnGap);
        yPosition = bottomMargin + (3 - row) * (axisHeight + rowGap);

        ax = axes('Parent', fig, ...
            'Position', [xPosition yPosition axisWidth axisHeight]);
        axesGrid(row, s) = ax;
        ...
        switch row
            case 1
                plotWaveField(ax, longitude, latitude, ...
                    selected(s).hsYear0, selected(s).directionYear0, ...
                    restorationMask, bathymetry, longitudeLimits, latitudeLimits, ...
                    hsLimits, arrowSpacing, arrowScale, arrowLineWidth, ...
                    bathyLevels, bathyColor, bathyLineWidth, ...
                    false, showLatitude, fontAxis, fontLabel, ...
            ...
            case 2
                plotWaveField(ax, longitude, latitude, ...
                    selected(s).hsRestored, selected(s).directionRestored, ...
                    restorationMask, bathymetry, longitudeLimits, latitudeLimits, ...
                    hsLimits, arrowSpacing, arrowScale, arrowLineWidth, ...
                    bathyLevels, bathyColor, bathyLineWidth, ...
                    false, showLatitude, fontAxis, fontLabel, ...
            ...
            case 3
                plotDifferenceField(ax, longitude, latitude, ...
                    selected(s).boundaryEquivalent, restorationMask, bathymetry, ...
                    longitudeLimits, latitudeLimits, boundaryEquivalentLimits, ...
                    bathyLevels, bathyColor, bathyLineWidth, ...
                    showLongitude, showLatitude, fontAxis, fontLabel, outlineColor);

                if showColorbar
                    addColorbar(ax, 'Boundary-equivalent \DeltaH_s (m)', fontAxis);
                end
        end
...
addRowLabel(fig, axesGrid(1,1), 'Restoration Year 0', fontRow);
addRowLabel(fig, axesGrid(2,1), ...
    sprintf('Restoration Year %d', yearRestored), fontRow);
addRowLabel(fig, axesGrid(3,1), ...
    'Boundary-equivalent \DeltaH_s (m)', fontRow);

sgtitle(fig, sprintf('%s spatial response: %s (%d%% survival)', ...
    speciesLabel, densityLabels{densityIndex}, survivorship(densityIndex)), ...
    'FontSize', 14, 'FontWeight', 'bold', 'Interpreter', 'tex');

drawnow;

outputBase = sprintf('Figure%d_Acropora_spatial_boundary_equivalent_%s_Year%d_GRIDMAX', ...
    figureNumber, densityShortNames{densityIndex}, yearRestored);

if savePNG
    exportgraphics(fig, fullfile(outputDir, [outputBase '.png']), ...
        'Resolution', 600);
end

if savePDF
    exportgraphics(fig, fullfile(outputDir, [outputBase '.pdf']), ...
        'ContentType', 'vector');
end
...
function [hsCell, directionCell, commonTimes] = loadHsDirection(runPath)
    ...
    validHs = cellfun(@(x) isnumeric(x) && ~isscalar(x), hsValues);
    validDirection = cellfun(@(x) isnumeric(x) && ~isscalar(x), directionValues);
    ...
    [commonTimes, hsIndex, directionIndex] = intersect(hsTimes, directionTimes);
    if isempty(commonTimes)
        error('No common timestamps between hsig.mat and wdir.mat in:\n%s', runPath);
    end
...
function times = parseTimesFromNames(names)
    ...
    if foundTimestamp
        ...
        times = datetime(timeStrings, 'InputFormat', 'yyyyMMdd_HHmmss');
    else
        times = (1:count)';
    end
end
...
function values = getAtTime(valueCell, times, targetTime)
    ...
    if ~found
        if isdatetime(times)
            ...
            [~, index] = min(abs(double(times) - double(targetTime)));
        end
    end
...
function values = loadGridMatrix(filePath, nRows, nCols, referenceGrid)
    data = load(filePath);
...
function plotWaveField(ax, longitude, latitude, hs, direction, mask, bathymetry, ...
        longitudeLimits, latitudeLimits, colorLimits, arrowSpacing, arrowScale, ...
        arrowLineWidth, bathyLevels, bathyColor, bathyLineWidth, ...
        showLongitude, showLatitude, fontAxis, fontLabel, outlineColor, colormapName)
    ...
    if ~isempty(bathymetry)
        contour(ax, longitude, latitude, bathymetry, bathyLevels, ...
            'Color', bathyColor, 'LineWidth', bathyLineWidth);
    end

    if any(mask(:))
        contour(ax, longitude, latitude, mask, [1 1], ...
            'Color', outlineColor, 'LineWidth', 2.0);
    end
    ...
    quiver(ax, ...
        ...
        arrowScale, 'k', 'LineWidth', arrowLineWidth);

    formatMapAxis(ax, longitudeLimits, latitudeLimits, ...
        showLongitude, showLatitude, fontAxis, fontLabel);
end

function plotDifferenceField(ax, longitude, latitude, differenceField, mask, bathymetry, ...
        longitudeLimits, latitudeLimits, colorLimits, bathyLevels, bathyColor, ...
        bathyLineWidth, showLongitude, showLatitude, fontAxis, fontLabel, outlineColor)
...
function formatMapAxis(ax, longitudeLimits, latitudeLimits, ...
        showLongitude, showLatitude, fontAxis, fontLabel)
    ...
    if showLongitude
        xlabel(ax, 'Longitude (°)', 'FontSize', fontLabel);
    else
        ax.XTickLabel = {};
    end

    if showLatitude
        ylabel(ax, 'Latitude (°)', 'FontSize', fontLabel);
    else
        ax.YTickLabel = {};
    end
end
...
function colorLimits = hurricaneColorLimits(hsYear0, hsRestored, percentileValue)
...
function addRowLabel(fig, referenceAxis, labelText, fontSize)
    axisPosition = referenceAxis.Position;

    labelAxis = axes('Parent', fig, ...
        'Position', [0.020 axisPosition(2) 0.045 axisPosition(4)], ...
        'Visible', 'off');
...
function map = whiteRed(count)
    if nargin < 1
        count = 256;
    end
    map = [ones(count,1), linspace(1,0,count)', linspace(1,0,count)'];
end
